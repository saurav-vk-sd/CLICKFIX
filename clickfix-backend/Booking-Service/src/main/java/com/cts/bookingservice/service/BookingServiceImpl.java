...
//			user = restTemplate.getForObject(url, UserDto.class);
    ...
    public void completePayment(int bookingId) {
    	int invoiceId = invoiceFeignClient.getInvoiceId(bookingId);
    	invoiceFeignClient.completeTransaction(invoiceId);
    	BookingDto booking = getBookingById(bookingId);
    	booking.setStatus("Paid");
    	Booking res = modelMapper.map(booking, Booking.class);
